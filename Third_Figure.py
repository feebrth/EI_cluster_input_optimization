...
grid = fig.add_gridspec(5, 5, left=0.10, right= 0.95, top=0.98, bottom=0.15, height_ratios=[4.0, 0.1, 1.5, 2.3, 2.3], hspace=0.4, wspace=0.3)

# Pareto-Front-Plot
pareto_ax = fig.add_subplot(grid[0, :])
pareto_ax.scatter(loss_ff, loss_rates, color="blue", label='Trials', alpha=0.9, s= 70)
pareto_ax.set_xlabel("Loss FF", fontsize=25)
pareto_ax.set_ylabel("Loss Rates", fontsize=25)
...
for col, (trial, label) in enumerate(zip(selected_trials, labels)):
    ...
    if col == 0:
        axs_fr.set_ylabel(r'$\Delta$ FR [spikes/s]', fontsize=25)
    else:
...
fig.text(x_position, 0.30, "(d)", fontsize=24, fontweight="bold", ha="right", va="center", rotation=0)
