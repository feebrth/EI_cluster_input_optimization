...
# Pareto-Front-Plot
pareto_ax = fig.add_subplot(2, 1, 1)
pareto_ax.scatter(loss_ff, loss_rates, color="blue", label="Trials", alpha=0.6, s=80)
pareto_ax.set_xlabel("Loss FF", fontsize=14)
pareto_ax.set_ylabel("Loss Rates", fontsize=14)
pareto_ax.set_title("Pareto-Front Plot", fontsize=18, fontweight="bold")
...
for i, (ff, rate) in enumerate(zip(selected_ff, selected_rates)):
    pareto_ax.scatter(ff, rate, color="red", s=150)
    pareto_ax.text(ff, rate, labels[i], fontsize=14, fontweight="bold", color="black", ha="right")

pareto_ax.legend(fontsize=12, loc="upper right")

# Subplots für Stimuli, Delta Firing Rates und Delta Fano Factors
grid = fig.add_gridspec(3, 5, top=0.65, height_ratios=[1, 3, 3])
...
for col, (trial, label) in enumerate(zip(selected_trials, labels)):
    ...
    axs_stim = fig.add_subplot(grid[0, col])
    ...
    axs_fr = fig.add_subplot(grid[1, col])
    ...
    axs_ff = fig.add_subplot(grid[2, col])
