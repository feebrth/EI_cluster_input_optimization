...
#Neuronpopulations
    ...
    def create_stimulation(self):
        ...
        #Creation of currentsource
        for dir in range(self._params["n_clusters"]):
            if dir in self._params["experiment_stim"]:
                stim_times = self._params["experiment_stim"][dir]["stim_time"]
                stim_amps = self._params["experiment_stim"][dir]["stim_amps"]


                self._currentsources = [nest.Create("step_current_generator")]
                nest.Connect(self._currentsources[0], self._populations[0][dir])

                nest.SetStatus(
                    self._currentsources[0],
                    {
                        "amplitude_times": np.array(stim_times) + self._params["warmup"],
                        "amplitude_values": stim_amps
                    },
                )
                # #creation of step_current_generator for each cluster
                # current_source = nest.Create("step_current_generator", params= {
                #     "amplitude_times": stim_times,
                #     "amplitude values": stim_amps
                # })

               # nest.Connect(current_source, self._populations[0][dir])
