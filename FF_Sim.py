...
def preprocess_spiketimes(spiketimes):
    # Gives back dictionary with Neuron ID as key and a list with every spiketime
    neurons_dict = {}
...
def FF_across_all_directions_and_neurons_optimized(spiketimes, timepoints, directions, direction_range, window_size=400,
                                                   step_size=100):
    # Spike-Times vorverarbeiten, um schneller darauf zugreifen zu können
    neurons_dict = preprocess_spiketimes(spiketimes)
    ...
    # Hauptschleife durch Zeitfenster und Richtungen
    for window_start in windows:
        fano_factors_for_all_directions = []

        for dir_value in direction_range:
            # Filtern der relevanten Trials vorab
            relevant_trials = np.array([timepoints[i] for i, d in enumerate(directions) if d == dir_value])

            spike_counts_per_neuron = {neuron: [] for neuron in neurons_dict.keys()}

            for neuron, neuron_spikes in neurons_dict.items():
                # Spike-Zählungen in allen relevanten Trials und im aktuellen Fenster berechnen
                for trial_start in relevant_trials:
                    trial_window_start = trial_start + window_start
                    trial_window_end = trial_window_start + window_size

                    # Alle Spikes im aktuellen Zeitfenster zählen
                    spike_count = np.sum((neuron_spikes >= trial_window_start) & (neuron_spikes < trial_window_end))
                    spike_counts_per_neuron[neuron].append(spike_count)

            # Fano-Faktor-Berechnung
            fano_factors_for_window = [
                np.var(counts) / np.mean(counts) if len(counts) > 1 and np.mean(counts) > 0 else 0
                for counts in spike_counts_per_neuron.values()
            ]
...
def plot_fano_factors(avg_fano_factor_curve, time_axis, exp_time, exp_ff):
    ...
    plt.title("Fano Factor Comparison: Simulation vs Experiment")
    plt.legend()
    plt.savefig("FF_comparison_50_trials.png")
...
def simulate_model(experimental_trials, direction_range, stim_kernel, kernel_step, plot=False):
    ...
    spiketimes = result["spiketimes"].T
    avg_fano_factor_curve, time_axis = FF_across_all_directions_and_neurons_optimized(spiketimes, timepoints, direction,
                                                                                      direction_range)
...
if __name__ == "__main__":
    stim_kernel = np.array([0.173, 0.826, 0.442, 0.356, 0.286, 0.304, 0.237, 0.247, 0.315, 0.379,
                            0.367, 0.395, 0.263, 0.328, 0.128, 0.0, 0.0, 0.0, 0.322, 0.497,
                            1.0, 1.0, 1.0, 1.0])
